...
import { categorySubToolService, ICategorySubTool } from "../../services/category-subtool.service";
...
export default function Categories() {
    const [categories, setCategories] = useState<ICategorySubTool[]>([]);
    const [searchTerm, setSearchTerm] = useState<string>("");
    const [showModal, setShowModal] = useState<boolean>(false);
    const [animateModal, setAnimateModal] = useState<boolean>(false);
    const [editingCategory, setEditingCategory] = useState<ICategorySubTool | null>(null);
    ...
    const loadCategories = async (): Promise<void> => {
        try {
            setLoading(true);
            const response = await categorySubToolService.getAll();
    ...
    const openModal = (category: ICategorySubTool | null = null): void => {
        if (category) {
    ...
    const handleSubmit = async (): Promise<void> => {
        ...
        try {
            ...
            if (editingCategory) {
                ...
                const response = await categorySubToolService.update(categoryId, formData);
                if (response.success) {
                    toast.success(response.message || "Cập nhật danh mục thành công");
                }
            } else {
                const response = await categorySubToolService.create(formData);
                if (response.success) {
    ...
    const handleDelete = async (id: string, name: string): Promise<void> => {
        ...
        try {
            setLoading(true);
            const response = await categorySubToolService.delete(id);
            if (response.success) {
