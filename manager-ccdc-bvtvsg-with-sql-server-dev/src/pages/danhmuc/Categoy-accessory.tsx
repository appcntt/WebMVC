...
import { categoryAccessoryService, ICategoryAccessory } from "../../services/category-accessory.service";
...
export default function Categories() {
    const [categories, setCategories] = useState<ICategoryAccessory[]>([]);
    const [searchTerm, setSearchTerm] = useState<string>("");
    const [showModal, setShowModal] = useState<boolean>(false);
    const [animateModal, setAnimateModal] = useState<boolean>(false);
    const [editingCategory, setEditingCategory] = useState<ICategoryAccessory | null>(null);
    ...
    const loadCategories = async (): Promise<void> => {
        try {
            setLoading(true);
            const data = await categoryAccessoryService.getAll();
            setCategories(data);
        } catch (error: any) {
    ...
    const openModal = (category: ICategoryAccessory | null = null): void => {
        if (category) {
    ...
    const handleSubmit = async (): Promise<void> => {
        ...
        try {
            ...
            if (editingCategory) {
                ...
                const response = await categoryAccessoryService.update(categoryId, formData);
                if (response.success) {
                    toast.success(response.message || "Cập nhật danh mục thành công");
                }
            } else {
                const response = await categoryAccessoryService.create(formData);
                if (response.success) {
    ...
    const handleDelete = async (id: string, name: string): Promise<void> => {
        ...
        try {
            setLoading(true);
            const response = await categoryAccessoryService.delete(id);
            if (response.success) {
